...
└── results/
    ├── table_summary.csv
    ├── table_ba_centrality.csv
    ├── table_tipping_scan.csv
    └── reproducibility_config.json
```

The directories `figures/` and `results/` are part of the reproducibility package and are intentionally versioned. The PNG files are the figures used in the manuscript, while the CSV and JSON files contain the corresponding numerical outputs and the exact simulation configuration.
...
By default, the script writes the manuscript figures to `figures/` and the numerical/reproducibility outputs to `results/`.
...
Custom output directories can be selected with

```bash
python scripts/run_experiments.py --figures-dir my_figures --results-dir my_results
```

For backward compatibility, `--output` is accepted as an alias for the figures directory.
...
![Final network snapshots](figures/fig_network_snapshots.png)
...
![Integration metrics across network topologies](figures/fig_topology_metrics.png)

### Cultural-broker potential

Evolution of the mean value of the ten largest integrator indices.

![Evolution of cultural-broker potential](figures/fig_integrator_evolution.png)

### Structural signatures

Final clustering coefficient and average path length for the three topologies.

![Structural signatures of final networks](figures/fig_topology_structure.png)
...
![Migrant-fraction sensitivity scan](figures/fig_tipping.png)
...
1. use the fixed random seeds recorded in `results/reproducibility_config.json`;
2. report the topology (`er`, `sw`, or `ba`);
3. report the population size and group composition;
4. report the model parameters recorded in `results/reproducibility_config.json`;
5. retain the CSV outputs in `results/` together with the generated figures;
6. regenerate figures and numerical tables through `scripts/run_experiments.py`.
